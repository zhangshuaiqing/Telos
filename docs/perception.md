...
1. [数据生命周期](#0-数据生命周期)
2. [通用接口](#1-通用接口)
3. [VisionChannel — 视觉通道](#2-visionchannel--视觉通道)
4. [VoiceChannel — 语音通道](#3-voicechannel--语音通道)
5. [ProprioChannel — 本体感通道](#4-propriochannel--本体感通道)
6. [PerceptionManager — 多通道汇集](#5-perceptionmanager--多通道汇集)
7. [扩展通道模板](#6-扩展通道模板)
8. [通道自检与降级](#7-通道自检与降级)
9. [多模态融合发送策略](#8-多模态融合发送策略)
...
│                      感知与记忆的分工                         │
│                                                             │
│  感知层 (Perception)          记忆系统 (Memory, Ch2)          │
│  ──────────────────          ──────────────────────           │
│  持续采集原始数据             选择性保留有意义的信息            │
│  像人的感官:                 像人的记忆:                      │
│    眼睛一直在看                 只记住值得注意的事             │
│    耳朵一直在听                 遗忘不重要的细节               │
│    身体一直有感觉              重要的反复强化                 │
│                                                             │
│  start() → 持续采集           感觉缓冲: 最后1-3帧原样保留     │
│  capture() → 返回最新快照     工作记忆: Agent 选择保留3-7条    │
│  不负责保留历史                情景记忆: 关键事件轨迹持久化    │
│                                 程序记忆: 习得的技能编码       │
└─────────────────────────────────────────────────────────────┘
```
...
```
你的眼睛: 一直在看 (Perception.start() → 持续采集)
你注意到什么: Agent 循环决定什么时候 "看" (capture())
你记得什么:   记忆系统决定保留什么 (Memory.record())

例: 你在开车
  眼睛一直在看路 (VisionChannel 30fps 持续采集)
  你每隔几秒关注一下后视镜 (Agent 500ms 间隔 capture)
  你不会记得路上每一棵树 (记忆系统选择性丢弃)
  但你会记得差点撞到的那辆红色卡车 (关键事件 → 情景记忆)
```

### 0.3 数据流向

```
硬件传感器 → Perception.start() → 持续采集 → 环形缓冲(最新N帧)
                                               │
                          Agent 决定采样 ────────┘
                                               │
                                          capture() → Observation
                          ...
                          ┌────────────────────┼──────────────────┐
                          ▼                    ▼                  ▼
                    感觉缓冲 (1-3帧原样)   工作记忆 (Agent选)   情景记忆 (关键事件)
                        自动覆盖             3-7条               SQLite持久化
                          │                    │                  │
                          └──────── 遗忘 ──────┴──────────────────┘
                                    (不重要的丢弃)
...
| 通道 | start() 后 | 内部缓冲 | capture() |
|------|-----------|---------|-----------|
| Vision | 后台线程 30fps 采集 → 环形缓冲最新 3 帧 | 3 帧 | 取最新帧压缩编码 |
| Voice | VAD 循环持续运行 → 最新识别文本 | 1 条文本 | 返回当前文本 |
| Proprio | STM32 线程 100Hz 推送 → 最新状态 | 1 份快照 | 返回当前快照 |
| Lidar(未来) | 串口持续读取 → 环形缓冲最近 10 个扫描周期 | 10 帧 | 取最新扫描 |

**关键设计:** capture() 总是返回最新数据，不积累历史。历史由记忆系统按自己的规则选择性保留。

### 0.5 不丢失关键事件

虽然感知层不保存历史，但关键事件不会丢失——由 Ch8 的 `SendDecisionEngine` 负责：

```
VoiceChannel 持续监听
  → 用户说 "急停" 
  → VAD 检测到语音 → ASR 识别 → text = "急停"
  → SendDecisionEngine.should_send() 检测到 is_critical=True
  → 立即触发 capture() → Observation → LLM → 急停动作

这个过程中 text 只在 VoiceChannel 内部保留了一瞬间，
但因为 SendDecisionEngine 即时响应，指令不会丢失。
```
